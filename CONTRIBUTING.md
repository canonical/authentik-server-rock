...
```shell
sudo /snap/rockcraft/current/bin/skopeo --insecure-policy copy oci-archive:<local-rock-name>.rock docker-daemon:authentik-server:latest
```

Run an Authentik server container using Docker:
